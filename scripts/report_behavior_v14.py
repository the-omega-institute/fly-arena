"""Compact actual-state comparison and noncircular v14 evidence closure."""
import argparse
import json
...
from flyarena.experiments.mechanical_v14 import PROFILE, CONTROL, validate_freeze
...
def main():
    ...
    for row,(profile,case,label) in enumerate([(CONTROL,'straight-02','Unchanged v12'),(PROFILE,'straight-02','V14 sensed-clearance feedback')]):
    ...
    fig.suptitle('V14 actual endpoint clearance and interval support — seed 42\n0.6–1.0 s excerpt; qualification uses every registered complete cycle')
    figure=root/'actual-clearance-support.png';fig.savefig(figure,dpi=150);plt.close(fig)
    rows=['# V14 whole-foot retraction feedback mechanical experiment','',f"Development candidate passed: **{analysis['candidate_passed']}**. Independent numerical verification passed: **{verified['passed']}**.",'','One prospectively fixed controller replaced the native single-leg relative-height retraction selector with independent deficient-swing whole-foot triggers. Native retraction rates, persistence, correction vectors, phase envelope, body, v12 timing/excursion, adhesion, bounds and gates were preserved. The .05 mm sensed setpoint is an engineered trigger; actual recovery still requires the unchanged >.02 mm unloaded protraction gate.','','| Profile | Seed | Case | Speed mm/s | Yaw rad | Recovery | Support/slip | Stop | Failed gates |','|---|---:|---|---:|---:|---|---|---|---|']
    ...
    (repo/'docs/BEHAVIOR_V14_RESULTS.md').write_text(report.replace('(actual-clearance-support.png)','(evidence/behavior-v14-actual-clearance-support.png)'))
    (repo/'docs/evidence').mkdir(exist_ok=True);shutil.copyfile(figure,repo/'docs/evidence/behavior-v14-actual-clearance-support.png')
    write_json(root/'resource-closure.json',{'schema':'behavior-v14-resources/v1','execution':terminal['resources'],'physics_seconds':terminal['resources']['mechanical_seconds'],'neural_seconds':0,'workers':1,'closed_after_semantic_verification':True,'registration_sha256':args.trusted_registration_sha256,'scientific_admission':False})
    ...
    write_json(root/'output-inventory.json',{'schema':'behavior-v14-output-inventory/v1','registration_sha256':args.trusted_registration_sha256,'payload':payload,'payload_files':len(payload),'payload_bytes':sum(v['bytes'] for v in payload.values()),'self_exclusion':'output-inventory.json; parent binds hash and closed size externally','no_files_may_be_added_after_inventory':True})
