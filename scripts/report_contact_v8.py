...
def summary(curve):
    return {'signed_mean':np.trapezoid(curve,dx=.01,axis=0).tolist(),'absolute_peak':np.max(np.abs(curve),axis=0).tolist(),'rms':np.sqrt(np.mean(curve**2,axis=0)).tolist(),'curve':curve.tolist()}
...
def main():
    ...
    for c in reg['conditions']:
        ...
        records[name]={'condition':c,'time':(a['ticks']*.0001).tolist(),'q':a['knees'].tolist(),'mn':antagonist(a).tolist(),
            ...
            'ratio':np.vstack([u['contact_ratios'],u['contact_ratios'][-1]]).tolist(),
    ...
    with (BASE/'derived.json').open('x') as f:json.dump(payload,f,allow_nan=False)
...
el('download').onclick=()=>{let a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(data)],{type:'application/json'}));a.download='contact-v8-derived-arrays.json';a.click();URL.revokeObjectURL(a.href)};render();
    ...
    with (BASE/'viewer.html').open('x') as f:f.write(template.replace('PAYLOAD',json.dumps(payload,allow_nan=False).replace('</','<\\/')))
    ...
    fig.savefig(BASE/'joint-phenotype.png',dpi=160);plt.close(fig)
    ...
    with (BASE/'report-check.json').open('x') as f:json.dump(facts,f,indent=2)
