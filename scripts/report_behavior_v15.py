"""Compact actual-state comparison and noncircular v15 evidence closure."""
import argparse
import json
...
from flyarena.experiments.mechanical_v15 import PROFILE, CONTROL, validate_freeze
...
def main():
    ...
    for row,(profile,case,label) in enumerate([(CONTROL,'straight-02','Unchanged v14'),(PROFILE,'straight-02','V15 normal allocation')]):
    ...
    (repo/'docs/BEHAVIOR_V15_RESULTS.md').write_text(report.replace('(actual-clearance-support.png)','(evidence/behavior-v15-actual-clearance-support.png)'))
    (repo/'docs/evidence').mkdir(exist_ok=True);shutil.copyfile(figure,repo/'docs/evidence/behavior-v15-actual-clearance-support.png')
    write_json(root/'resource-closure.json',{'schema':'behavior-v15-resources/v1','execution':terminal['resources'],'physics_seconds':terminal['resources']['mechanical_seconds'],'neural_seconds':0,'workers':1,'closed_after_semantic_verification':True,'registration_sha256':args.trusted_registration_sha256,'scientific_admission':False})
    ...
    write_json(root/'output-inventory.json',{'schema':'behavior-v15-output-inventory/v1','registration_sha256':args.trusted_registration_sha256,'payload':payload,'payload_files':len(payload),'payload_bytes':sum(v['bytes'] for v in payload.values()),'self_exclusion':'output-inventory.json; parent binds hash and closed size externally','no_files_may_be_added_after_inventory':True})
