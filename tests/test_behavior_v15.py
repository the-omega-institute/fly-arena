"""Registered nonphysical v15 law, native equivalence, atomicity and cache checks."""
import copy
...
from flyarena.experiments.cadence_v14 import ClearanceHybridController, ClearanceObservation
from flyarena.experiments.cadence_v15 import AllocationHybridController,AllocationObservation,normal_allocation,PROFILE,CONTROL
from flyarena.experiments import mechanical_v15 as runner
from flyarena.experiments.verify_v15 import verify_allocation_row
...
def test_minimum_norm_tangent_covariance_and_inherited_budget():
    ...
    for norm in (.06,np.sqrt(.001651),.03)*2:
        ...
        x,d=normal_allocation(J,0,q,q,np.full(7,-np.inf),np.full(7,np.inf),b)
        ...
        y,_=normal_allocation(R@J,0,q,q,np.full(7,-np.inf),np.full(7,np.inf),b)
...
def test_rank_and_authority(rank):
    ...
    x,d=normal_allocation(J,0,np.zeros(7),np.zeros(7),np.full(7,-np.inf),np.full(7,np.inf),1)
    ...
    x,d=normal_allocation(J,.05,np.zeros(7),np.zeros(7),np.full(7,-np.inf),np.full(7,np.inf),1)
...
def test_exact_rank_threshold():
    ...
    _,d=normal_allocation(J,0,np.zeros(7),np.zeros(7),np.full(7,-np.inf),np.full(7,np.inf),1)
    assert d[6]==1
...
def test_compiled_limit_budget_and_tracking(sign):
    ...
    x,d=normal_allocation(J,0,q,q0,lo,hi,1)
    assert abs(x[2]-.03*sign)<1e-15 and abs(d[5]-.04)<1e-15 and d[7]==2
    x,d=normal_allocation(J,0,q,q0,lo,hi,.01)
...
def test_native_trace_persistence_priority_and_saturation_equivalence():
    c=controller();v=ClearanceHybridController(timestep=runner.DT);v.reset(seed=42)
    ...
    for tick in range(60):
        ...
        c.step([.12,.28],obs)
        v.step([.12,.28],ClearanceObservation(obs.native,obs.clearance,obs.minimum_geom,obs.minimum_vertex))
...
def test_updated_strict_phase_and_stumbling_branch(phase):
    ...
    with patch.object(type(c.cpg_network),'step',lambda self:None):
        ...
        expected=v.step([.2,.2],ClearanceObservation(obs.native,obs.clearance,obs.minimum_geom,obs.minimum_vertex))
...
def test_panel_count_and_no_source_control_substitution():
    assert CONTROL=='whole-foot-retraction-feedback-v14'
...
def test_recorded_controller_equations_and_tamper_detection(profile):
    from flyarena.experiments.verify_v15 import verify_phase_law,verify_retraction_and_commands
    c=controller() if profile==PROFILE else ClearanceHybridController(timestep=runner.DT)
    if profile==CONTROL:c.reset(seed=42)
    ...
    for k in range(1,31):
        obs=observation(k-1)
        c.step([.2,.2],obs if profile==PROFILE else ClearanceObservation(obs.native,obs.clearance,obs.minimum_geom,obs.minimum_vertex))
