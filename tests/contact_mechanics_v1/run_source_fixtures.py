"""Single guarded source fixture process; no native execution."""
import ast,importlib.util,io,json,pathlib,resource,sys,time,unittest
sys.dont_write_bytecode=True
root=pathlib.Path(__file__).resolve().parents[2];scratch=pathlib.Path('/tmp/fly-contact-mechanics-fix1');scratch.mkdir(exist_ok=True)
...
suite=unittest.TestSuite([unittest.defaultTestLoader.loadTestsFromModule(module),unittest.defaultTestLoader.loadTestsFromModule(module2)])
...
(scratch/'fixtures.txt').write_text(stream.getvalue());(scratch/'fixture-report.json').write_text(json.dumps(report,indent=2)+'\n');print(stream.getvalue());print(json.dumps(report))
