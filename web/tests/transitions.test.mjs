...
test('slow replay clears ready A immediately and commits only the complete B pair',async()=>{
  ...
  net.calls[1].resolve(frames(2));await request.done
...
for(const failIndex of [0,1])test(`replay endpoint ${failIndex} failure discards partial data and aborts its sibling`,async()=>{
  ...
  net.calls[1-failIndex].resolve(failIndex?scene('B'):frames(2));await Promise.resolve()
...
test('A → B → C cancellation rejects late success/error even if transport ignores abort',async()=>{
  ...
  assert.ok(net.calls.slice(0,4).every(c=>c.options.signal.aborted))
  assert.ok(net.calls.slice(4).every(c=>!c.options.signal.aborted))
  net.calls[4].resolve(scene('C'));net.calls[5].resolve(frames(3));await c.done
  net.calls[0].resolve(scene('A'));net.calls[1].resolve(frames(1));await a.done
  net.calls[2].reject(new Error('obsolete 503'));net.calls[3].resolve(frames(2));await b.done
...
test('cancel on unmount/queued focus produces no error or ready commit and permits same-ID retry',async()=>{
  ...
  net.calls[2].resolve(scene('A-new'));net.calls[3].resolve(frames(7));await retry.done
  net.calls[0].reject(new Error('old A'));net.calls[1].resolve(frames(1));await first.done
...
test('empty frames leave an unavailable replay, never a verified scene',async()=>{
  ...
  net.calls[0].resolve(scene('A'));net.calls[1].resolve([]);await request.done
