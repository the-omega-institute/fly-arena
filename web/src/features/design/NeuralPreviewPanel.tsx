...
export function NeuralPreviewPanel({record,stale,circuits,onImport,onReplay}:{record:PreviewRecord;stale:boolean;circuits:Circuit[];onImport:(spec:Spec)=>void;onReplay:(id:string)=>void}){
 ...
 return <section className="panel stimulus-preview" aria-label={zh?'设计与 WT 神经响应':'Design and WT neural response'}>
  ...
  <div className="stimulus-preview__body">
   ...
   {example&&p.example?.same_motor_readout_as_related_match===false&&<p className="stimulus-preview__notice">{zh?'关联比赛使用相同大脑设计，但动作解码器校准版本不同。这里的刺激响应不能直接解释那场比赛的运动。':'The linked contest uses the same brain design with a different motor decoder calibration. This stimulus response does not reconstruct that contest’s motion.'}</p>}
   ...
   <details><summary>{zh?'查看本次设计与实验条件':'Inspect design and experiment conditions'}</summary><pre>{JSON.stringify({artifact_id:p.artifact_id,connectome:p.connectome_sha256,reference:p.reference,protocol:p.protocol,motor_readout:p.motor_readout,example:p.example,spec:p.spec,scope:p.scope},null,2)}</pre></details>
  </div>
 </section>
}
