...
export function PlaygroundGuide({onDesign,onTrain,onArena,onAI,onCloneWT,canCloneWT,hasSavedDesign,hasDraft=false,referenceOnly=false,neuronCount,onCompare,canCompare,hasCompared=false,hasEvolved=false,subjectName,evidenceError}:PlaygroundGuideProps){
  ...
  const next=nextGuideStep({hasSavedDesign,hasDraft,hasCompared,hasEvolved})
  ...
  if(!expanded&&!referenceOnly)return <section className="playground-guide playground-guide--collapsed" aria-label={t('guide.label')}><BookOpen size={17}/><span>{t('guide.next')}</span>{nextAction}<button className="playground-guide__reopen" onClick={reopen} aria-label={t('guide.reopen')}><ChevronDown size={16}/></button></section>

  return <section className="playground-guide" aria-label={t('guide.label')}>
    {!referenceOnly&&<><div className="playground-guide__topline"><span className="playground-guide__eyebrow"><span/> {t('guide.eyebrow')}</span><button className="playground-guide__collapse" onClick={collapse} aria-label={t('guide.collapse')}><ChevronUp size={16}/></button></div>
    ...
    <details className="playground-guide__library"><summary>{t('guide.library')}</summary><p>{t('guide.workspaces')}</p>
