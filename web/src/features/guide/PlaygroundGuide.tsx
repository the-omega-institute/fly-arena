...
export function PlaygroundGuide({onDesign,onTrain,onArena,onAI,onCloneWT,canCloneWT,hasSavedDesign,neuronCount}:PlaygroundGuideProps){
  ...
  return <section className="playground-guide" aria-labelledby="playground-guide-title">
    ...
    <details className="playground-guide__science">
      ...
      <div className="playground-guide__pipeline"><div><b>01</b><strong>{copy('Real connectome source','真实图谱来源')}</strong><p>{copy('The pinned MaleCNS v1.0 connectome supplies the graph structure. EM images are reconstructed into neurons and synapse partners; this is not a live animal.','固定的 MaleCNS v1.0 神经图谱提供网络结构。电子显微镜图像被重建为神经元和突触配对；它不是活体动物。')}</p><a href="https://male-cns.janelia.org/" target="_blank" rel="noreferrer">{copy('MaleCNS source','MaleCNS 来源')} <ExternalLink size={12}/></a></div><ArrowRight size={15}/><div><b>02</b><strong>{copy('Simulated neuron dynamics','模拟神经元动力学')}</strong><p>{copy('The retained graph runs with your selected LIF spiking model or experimental continuous-rate model. Structural synapse counts are not experimentally known physiological weights.','保留的图谱使用你选择的 LIF 脉冲模型或实验性连续发放率模型运行。结构突触数量不等于实验测得的生理权重。')}</p></div><ArrowRight size={15}/><div><b>03</b><strong>{copy('Sensory to motor bridge','感觉到运动的连接')}</strong><p>{copy('Odor-only input is the default. Experimental profiles can add geometric visual observations and food/environment contact currents. A shared engineered readout drives two bilateral locomotion channels.','默认仅接收气味输入；实验模式还可接收几何视觉观测和食物、环境接触电流。共享的工程化读出驱动两个双侧运动通道。')}</p></div><ArrowRight size={15}/><div><b>04</b><strong>{copy('Body and records','身体与记录')}</strong><p>{copy('FlyGym/NeuroMechFly body behavior is stepped in MuJoCo. Runs record neural traces, motion, events, scores and replay evidence.','FlyGym/NeuroMechFly 身体行为在 MuJoCo 中推进。运行会记录神经曲线、运动、事件、分数和回放证据。')}</p><a href="https://github.com/NeLy-EPFL/flygym" target="_blank" rel="noreferrer">{copy('FlyGym source','FlyGym 来源')} <ExternalLink size={12}/></a></div></div>
      <p className="playground-guide__boundary">{copy('WT is the unmodified simulated baseline. The model is not identical to a real animal: experimental visual input is not a retina model; online learning and memory tasks are not enabled. The low level gait is engineered rather than learned. Anatomical locations and connections are source data; activity is produced by the simulation.','WT 是未修改的模拟基线。该模型不等同于真实动物：实验性视觉输入不是视网膜模型，在线学习和记忆任务尚未启用，低层步态也是工程化的。解剖位置和连接来自原始图谱，活动则由仿真产生。')}</p>
    </details>
  </section>
}
