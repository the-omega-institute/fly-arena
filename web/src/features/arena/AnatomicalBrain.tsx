...
export function AnatomicalBrain({connectome,graph,activity,scale,frames,slot,time,onSeek,focusId,onFocus}:{connectome?:string;graph:NeuralGraph;activity:Map<string,number>;scale:number;frames:Frame[];slot:number;time:number;onSeek:(time:number)=>void;focusId?:string|null;onFocus?:(id:string)=>void}){
  ...
  return <div className="anatomical-brain">
    ...
    {current&&space?<>
      ...
      <div className="anatomical-brain-stage"><CanvasBoundary key={connectome} fallback={noWebgl}><AnatomicalBrainCanvas space={space} nodes={nodes} graph={graph} focus={chosen} onFocus={selectFocus} scale={scale} angle={angle} reset={reset}/></CanvasBoundary></div>
