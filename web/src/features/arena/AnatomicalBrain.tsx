...
export function AnatomicalBrain({connectome,graph,activity,scale,frames,slot,time,onSeek,focusId,onFocus,onMorphologyReady,playback}:{playback?:{playing:boolean;onToggle:()=>void};connectome?:string;graph:NeuralGraph;activity:Map<string,number>;scale:number;frames:Frame[];slot:number;time:number;onSeek:(time:number)=>void;focusId?:string|null;onFocus?:(id:string)=>void;onMorphologyReady?:(ids:string[])=>void}){
  ...
  return <div className={'anatomical-brain'+(expanded?' morphology-expanded':'')}>
    ...
    {current&&space?<>
      ...
      {morphology?<div className="morphology-legend"><strong>{morphology.parcellation?(structure?(zh?'颜色 = 真实脑区 · 静态形态':'Color = anatomical region · static morphology'):(zh?'颜色 = 真实脑区 · 亮度 = 记录活动':'Color = anatomical region · brightness = recorded activity')):(zh?'灰色 = 未提供脑区标注':'Gray = region annotations unavailable')}</strong><span>{morphology.neurons.length} {zh?'条真实神经元骨架':'real neuron skeletons'} · {morphology.neurons.filter(n=>activity.has(n.id)).length} {zh?'条有本帧活动记录':'with activity records in this frame'}</span><p>{zh?'纤维来自 MaleCNS 官方 SWC，脑区来自官方三维分割，按纤维段中点所属体素着色。未采样神经元仅作暗色结构背景，不闪烁。这是选定神经元的形态与活动样本，尚非全脑活动成像；亮度表示记录的活动强度，不是逐次放电或轴突内传播。':'Fibers use official MaleCNS SWCs; region colors use the official 3D parcellation at segment midpoints. Unrecorded neurons remain dim structural context and never flash. This is a selected sample, not whole-brain activity imaging. Brightness represents recorded activity strength, not individual spikes or within-axon propagation.'}</p><a href="https://male-cns.janelia.org/download/" target="_blank" rel="noreferrer">MaleCNS · Berg et al., Cell (2026) · CC-BY 4.0</a></div>:<p className="anatomical-brain-note">{morphologyState==='loading'?(zh?'正在载入真实神经纤维…':'Loading real neural fibers…'):(zh?'本部署未提供神经纤维数据；下方为胞体与连接示意。':'Neuron fibers are unavailable on this deployment; showing somata and schematic connectivity.')}</p>}
