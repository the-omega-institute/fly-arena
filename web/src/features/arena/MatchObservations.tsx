...
export function BrainTheater({frame,frames,season,fly,slot,events,onSeek,activityScale,nodeScale,matchId}:{matchId?:string;frame?:Frame;frames:Frame[];season:Season|null;fly?:ReplayParticipant;slot:number;events:ReplayEvent[];activityScale:number;nodeScale:number;onSeek:(time:number)=>void}) {
  ...
  function selectCircuit(id:string){
    ...
    const useLocal=view==='anatomy'&&anchors.length>0&&anchors.every(n=>!Array.isArray(n.position)||n.position.length!==3||!n.position.every(Number.isFinite))
  ...
  return <section className="brain-theater panel" ref={brainHeading}>
    ...
    {view==='anatomy'&&(graph?<AnatomicalBrain connectome={fly?.spec.connectome_sha256} graph={spatialAll&&anatomyGraph?anatomyGraph:graph} focusId={focusId} onFocus={setFocusId} activity={activity} scale={nodeScale} frames={frames} slot={slot} time={frame?.time??0} onSeek={onSeek}/>:<p className="empty">{t('Loading canonical neuron sample…')}</p>)}
