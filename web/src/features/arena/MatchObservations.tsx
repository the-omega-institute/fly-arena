...
type ReplayReceipt={schema?:string;sha256?:string;connectome_sha256?:string;neuron_count?:number;edge_count?:number;request?:{bridge_profile?:string;sensory_profile?:string};runtime?:{rules?:{id?:string;feeding_contact?:string};machine?:string;python?:string;mujoco?:string;bridge_profile?:string;readout_weights_sha256?:string;model?:{id?:string};sensory_profile?:{id?:string}}}
...
function BrainTheater({frame,frames,season,fly,slot,events,onSeek,activityScale,nodeScale}:{frame?:Frame;frames:Frame[];season:Season|null;fly?:ReplayParticipant;slot:number;events:ReplayEvent[];activityScale:number;nodeScale:number;onSeek:(time:number)=>void}) {
  ...
  useEffect(()=>{let active=true;setGraph(null);setSelectedClass(null);const query='/connectome/neurons?circuit='+encodeURIComponent(circuit)+(sampleIds?'&ids='+encodeURIComponent(sampleIds)+'&limit=80':'&limit=80');api<Graph>(query).then(v=>{if(active)setGraph(v)}).catch(()=>{if(active)setGraph(null)});return()=>{active=false}},[circuit,sampleIds])
  ...
  return <section className="brain-theater panel">
    ...
    {view==='local'&&<div className="brain-graph-wrap">{graph?<svg className="brain-graph" viewBox="0 0 500 280" role="img" aria-label={t('Recorded neural activity graph')}>
