...
export function BrainTheater({frame,frames,season,fly,slot,events,onSeek,activityScale,nodeScale,matchId,playback}:{playback?:{playing:boolean;onToggle:()=>void};matchId?:string;frame?:Frame;frames:Frame[];season:Season|null;fly?:ReplayParticipant;slot:number;events:ReplayEvent[];activityScale:number;nodeScale:number;onSeek:(time:number)=>void}) {
  const {t,locale}=useI18n(); const [circuit,setCircuit]=useState('olfactory'); const [graph,setGraph]=useState<Graph|null>(null); const [designWeights,setDesignWeights]=useState(false); const [view,setView]=useState<BrainView>(()=>fly?.artifact_id&&/^[a-f0-9]{64}$/.test(fly.spec?.connectome_sha256||'')?'anatomy':'region'); const [selectedClass,setSelectedClass]=useState<string|null>(null); const [selectedEvent,setSelectedEvent]=useState<ReplayEvent|null>(null); const [responseMs,setResponseMs]=useState(100)
  ...
  const [morphologyIds,setMorphologyIds]=useState<string[]>([])
  ...
  const inspectionGraph=focusId&&anatomyGraph?.neurons.some(n=>n.id===focusId)?anatomyGraph:graph
  const recordedAnchors=nodes.filter(n=>activity.has(n.id)&&(!graph?.anchors?.length||graph.anchors.includes(n.id)))
  const missingAnchorPositions=recordedAnchors.length>0&&recordedAnchors.every(n=>!Array.isArray(n.position)||n.position.length!==3||!n.position.every(Number.isFinite))
  function selectCircuit(id:string){
    const neighborhood=snapshot?.circuits[id]
    const anchors=neighborhood?.neurons.filter(n=>activity.has(n.id)&&(!neighborhood.anchors?.length||neighborhood.anchors.includes(n.id)))||[]
    const useLocal=view==='anatomy'&&anchors.length>0&&anchors.every(n=>!morphologyIds.includes(n.id)&&(!Array.isArray(n.position)||n.position.length!==3||!n.position.every(Number.isFinite)))
    setFocusId(useLocal?anchors[0].id:null);setSelectedClass(null);setCircuit(id);setSpatialAll(false)
    setView(useLocal?'local':view==='anatomy'?'anatomy':'region')
  }
  ...
  return <section className="brain-theater panel" ref={brainHeading}>
    ...
    {view==='local'&&missingAnchorPositions&&<p className="training-hint brain-missing-coordinates">{locale==='zh-CN'?'这组已记录神经元缺少源数据解剖坐标。这里展示真实连接与活动曲线，节点位置仅为连接示意。':'Source anatomical coordinates are unavailable for this group’s recorded neurons. Explore their actual connections and activity here; node positions are schematic.'}</p>}
    ...
    {view==='local'&&(inspectionGraph?<LocalBrainGraph graph={inspectionGraph} focusId={focusId} onFocus={setFocusId} activity={activity} scale={nodeScale} selectedClass={selectedClass} renderActivity={neuronId=><NeuronActivityTrace neuronId={neuronId} frames={frames} slot={slot} time={frame?.time??0} onSeek={onSeek}/>}/>:<p className="empty">{t('Loading canonical neuron sample…')}</p>)}
    ...
    {view==='anatomy'&&(graph?<AnatomicalBrain events={events} selectedEvent={selectedEvent} playback={playback} onMorphologyReady={setMorphologyIds} connectome={fly?.spec.connectome_sha256} graph={spatialAll&&anatomyGraph?anatomyGraph:graph} focusId={focusId} onFocus={setFocusId} activity={activity} scale={nodeScale} frames={frames} slot={slot} time={frame?.time??0} onSeek={onSeek}/>:<p className="empty">{t('Loading canonical neuron sample…')}</p>)}
    {sample?.population&&<p className="training-hint">{locale==='zh-CN'?'全网络记录摘要':'Full-network summary'} · {sample.population.neuron_count.toLocaleString()} {locale==='zh-CN'?'神经元':'neurons'} · {locale==='zh-CN'?'平均':'mean'} {sample.population.mean_hz.toFixed(2)} Hz · {sample.population.active_count.toLocaleString()} &gt; 1 Hz · {sample.population.above_400hz_count.toLocaleString()} &gt; 400 Hz</p>}
...
export function MatchObservations({scene,frame,frames,events=[],flies,selectedId,season,match,onSeek=()=>{},onObserveFly,onDesignReplay,playback}:{playback?:{playing:boolean;onToggle:()=>void};scene:Scene;frame?:Frame;frames:Frame[];events?:ReplayEvent[];flies:Fly[];selectedId:string;season:Season|null;match?:Match;onSeek?:((time:number)=>void);onObserveFly?:((id:string)=>void);onDesignReplay?:(draft:Spec,origin:ReplayDesignOrigin)=>void}) {
  const {t,locale}=useI18n()
  const [slot,setSlot]=useState(()=>selectedReplaySlot(scene,selectedId))
  useEffect(()=>setSlot(selectedReplaySlot(scene,selectedId,slot)),[selectedId,scene.flies])
  ...
  function participantCard(index:number){
    ...
    const outcome=recordedOutcome(frames,index,match?.result)
    ...
    return <div className="observation-participant" key={entry?.id||index}>
      ...
      <section className="replay-flow-detail" aria-label={t('Recorded replay flow')}>
        <div className="replay-flow-heading"><strong>{t('Recorded event and behavior navigator')}</strong><span>{t('Current playhead')} · {shown(frame?.time,2)} s</span></div>
        <BrainTheater playback={playback} matchId={match?.id} frame={frame} frames={frames} season={season} fly={subjectFly} slot={index} events={visibleEvents} onSeek={onSeek} activityScale={activityScales.region} nodeScale={activityScales.node}/>
        <details className="replay-behavior-disclosure" open>
          <summary>{t('Body, sensory, neural and resource detail')}</summary>
          ...
          <div className="synchronized-observation-detail"><div><span>{t('Recorded body state')}</span><strong>{frame?.positions?.[index]?.map(value=>Number.isFinite(value)?value.toFixed(2):'—').join(' / ')||'—'} mm</strong></div><div><span>{t('Shared food and resource history')}</span><strong>{frame?.food?.length?frame.food.map(value=>Number.isFinite(value)?value.toFixed(3):'—').join(' · '):'—'}</strong></div></div>
        </details>
        <details className="replay-trace-disclosure">
          <summary>{t('Detailed traces, comparison and export')}</summary>
          <div className="observation-metrics"><div><span>{match?.request?.mode==='duel'?(locale==='zh-CN'?'中央独占时间 · 秒':'Exclusive center time · s'):t('Food collected')}</span><strong>{shown(frame?.scores?.[index],2)}</strong><svg viewBox="0 0 240 52" role="img" aria-label={match?.request?.mode==='duel'?(locale==='zh-CN'?'中央独占时间曲线':'Exclusive center time history'):t('Food score over time')}><path d={tracePath(frames,f=>f.scores?.[index],scorePeak)} fill="none" stroke={color} strokeWidth="2"/></svg></div><div><span>{t('Energy reserve')}</span><strong>{shown(frame?.energy?.[index])}</strong><svg viewBox="0 0 240 52" role="img" aria-label={t('Energy over time')}><path d={tracePath(frames,f=>f.energy?.[index],Math.max(100,...frames.map(f=>f.energy?.[index]??0)))} fill="none" stroke={color} strokeWidth="2"/></svg></div><div><span>{t('Left / right motor drive')}</span><strong>{shown(frame?.drives?.[index]?.[0],2)} / {shown(frame?.drives?.[index]?.[1],2)}</strong><small>{t('Recorded at')} {shown(frame?.time,2)} s</small></div></div>
          ...
          <button className="text-link replay-export" onClick={download}><ArrowDownToLine size={14}/>{t('Export records')}</button>
        </details>
      </section>
    </div>
  }
  ...
  return <section className="trace-panel panel observations">
    ...
    {request?.mode!=='sumo'&&request?.mode!=='duel'&&<details className="replay-analysis"><summary>{locale==='zh-CN'?'共享食物与资源变化':'Shared food and resource history'}</summary><SharedResources scene={scene} frame={frame} frames={frames} events={events} onSeek={onSeek} onObserveSlot={index=>{setCompare(false);setSlot(index);onObserveFly?.(scene.flies[index].id)}}/></details>}
