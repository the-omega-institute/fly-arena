...
function BrainTheater({frame,season,fly,slot,events,onSeek}:{frame?:Frame;season:Season|null;fly?:Fly;slot:number;events:ReplayEvent[];onSeek:(time:number)=>void}) {
  ...
  return <section className="brain-theater panel">
    ...
    <div className="sensory-strip"><div><span>{t('Odor L / R')}</span><strong>{frame?.senses?.[slot]?.odor.map(v=>v.toFixed(2)).join(' / ')||'—'}</strong></div><div><span>{t('Visual L / R')}</span><strong>{frame?.senses?.[slot]?.visual.map(v=>v.toFixed(2)).join(' / ')||'—'}</strong><small>{frame?.senses?.[slot]?.visual_status==='engineered_observation'?t('geometric observation'):''}</small></div><div><span>{t('Touch')}</span><strong>{frame?.senses?.[slot]?.touch?'ON':'—'}</strong><small>{frame?.senses?.[slot]?.touch_status==='geometric_contact_observation'?t('geometric observation'):''}</small></div><div><span>{t('Nearest food')}</span><strong>{frame?.senses?.[slot]?.nearest_food==null?'—':frame.senses[slot].nearest_food.toFixed(1)+' mm'}</strong></div></div>
    ...
    <div className="brain-graph-wrap">{graph?<svg className="brain-graph" viewBox="0 0 500 160" role="img" aria-label={t('Recorded neural activity graph')}>
      ...
      {nodes.map((n,i)=>{const p=point(i),value=activity.get(n.id)||0;return <g key={n.id}><circle cx={p.x} cy={p.y} r={value?4.5:2.7} fill={value?`hsl(${145-(value/max)*115} 75% 57%)`:'#7e8e80'} opacity={value?.9:.52}/><title>{n.id} · {n.type} · {value.toFixed(2)} Hz</title></g>})}
