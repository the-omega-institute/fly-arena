...
export function AnatomicalBrainCanvas({space,nodes,graph,focus,onFocus,scale,angle,reset,morphology,activity,structure=false,wholeCns=false,changeMode=false,recordedIds}:{space:AnatomySpace;nodes:SpatialNode[];graph:NeuralGraph;focus:string|null;onFocus:(id:string)=>void;scale:number;angle:BrainAngle;reset:number;morphology?:Morphology;activity?:Map<string,number>;structure?:boolean;wholeCns?:boolean;changeMode?:boolean;recordedIds?:string[]}){
  const neighbors=new Set(graph.edges.filter(e=>e.pre===focus||e.post===focus).flatMap(e=>[e.pre,e.post]))
  if(morphology&&activity)return <MorphologyCanvas recordedIds={recordedIds} changeMode={changeMode} data={morphology} activity={activity} focus={focus} onFocus={id=>{if(graph.neurons.some(n=>n.id===id))onFocus(id)}} scale={scale} angle={angle} reset={reset} structure={structure} wholeCns={wholeCns}/>
