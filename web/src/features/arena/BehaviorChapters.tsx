...
import {behaviorChapters} from './behaviorSummary'

export function BehaviorChapters({frames,events,slot,time,onSeek}:{
  frames:Frame[];events:ReplayEvent[];slot:number;time:number;onSeek:(time:number)=>void;
}){
  ...
  return <section className="behavior-chapters" aria-label={zh?'实际行为分段':'Recorded behavior chapters'}>
    <div className="behavior-chapters-heading"><div><strong>{zh?'这段生命记录发生了什么':'What happened in this life record'}</strong><p>{zh?'选择一段，从该时刻观察身体和大脑。':'Choose a chapter to observe its body and brain.'}</p></div>
      <span>{zh?'最后一次摄取事件':'Last intake event'}<b>{lastIntake===null?(zh?'未记录':'Not recorded'):lastIntake.toFixed(2)+' s'}</b></span></div>
    ...
    <p className="behavior-chapters-note">{zh?'每段约 5 秒，边界使用实际采样时间。路径为胸部 XY 采样位移之和，也可能包含滑动；摄取增量来自该回放的记账规则。接触数统计已记录事件，不表示接触时长；缺失数据用 — 表示。':'Chapters span about 5 seconds using recorded timestamps. Path sums sampled thorax XY motion, including possible slipping. Intake follows this replay’s rules. Contact counts are recorded events, not contact duration; — means missing data.'}</p>
  </section>
}
