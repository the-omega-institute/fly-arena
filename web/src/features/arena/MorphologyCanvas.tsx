...
const vertex=`attribute float neuron; attribute vec3 tint; uniform sampler2D states; uniform float width; varying vec4 state; varying vec3 hue;
void main(){state=texture2D(states,vec2((neuron+0.5)/width,0.5));hue=tint;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.0);gl_PointSize=1.4;}`
const fragment=`uniform float structure; varying vec4 state; varying vec3 hue;
void main(){bool recorded=state.g>0.5; float strength=state.r; vec3 c=hue;float alpha;
if(structure>0.5){alpha=0.40;}else if(recorded){alpha=0.035+0.90*strength; c=mix(hue*0.3,hue,strength);}else{c=vec3(0.24,0.32,0.36);alpha=0.10;}
...
function Fibers({data,activity,scale,focus,onFocus,structure,wholeCns,angle,reset}:{data:Morphology;activity:Map<string,number>;scale:number;focus:string|null;onFocus:(id:string)=>void;structure:boolean;wholeCns:boolean;angle:BrainAngle;reset:number}){
 ...
 const resources=useMemo(()=>{
  ...
  const material=new THREE.ShaderMaterial({vertexShader:vertex,fragmentShader:fragment,uniforms:{states:{value:texture},width:{value:data.neurons.length},structure:{value:0}},transparent:true,depthWrite:false,blending:THREE.AdditiveBlending})
 ...
 useEffect(()=>{const orbit=controls as unknown as {target:THREE.Vector3;update:()=>void}|null;camera.up.set(0,angle==='xz'?0:1,angle==='xz'?1:0);camera.position.set(...(angle==='xz'?[0,-3.3,0]:angle==='xy'?[0,0,-3.3]:[.7,-.3,-3.3]) as [number,number,number]);orbit?.target.set(0,0,0);camera.lookAt(0,0,0);orbit?.update();invalidate()},[camera,controls,angle,reset,wholeCns,invalidate])
 return <>
  ...
  <points geometry={resources.geometry} material={resources.material} raycast={()=>{}}/>
 </>
}
export function MorphologyCanvas(props:Parameters<typeof Fibers>[0]){
 return <Canvas frameloop="demand" dpr={[1,1.5]} camera={{fov:42,near:.01,far:30,position:[0,0,-3.3]}} gl={{antialias:true,alpha:false}} onCreated={({raycaster})=>{raycaster.params.Line.threshold=.006}}>
