...
export function ScenePlanView({world,recorded=false,frame,next,alpha=0,frames=[],selectedId,participants=[]}:{world?:PlanWorld|null;recorded?:boolean;frame?:Frame;next?:Frame;alpha?:number;frames?:Frame[];selectedId?:string;participants?:{name:string;color:string}[]}){
 ...
 const time=playbackTime(frame,next,alpha),half=world.size/2,unit=world.size/65
 ...
 return <div className="scene-plan-view">
  <svg className="scene-plan" viewBox={bounds.join(' ')} preserveAspectRatio="xMidYMid meet" role="img" aria-label={t(recorded?'scene.recordedPlan':'scene.layoutPlan')}>
   ...
   {spawns.map((spawn,i)=>Array.isArray(spawn)&&spawn.length>=3&&spawn.every(Number.isFinite)&&<g key={i} className="scene-plan-spawn" transform={`translate(${spawn[0]} ${-spawn[1]})`}><title>{`${t('scene.spawn')} ${i+1}${participants[i]?` · ${participants[i].name}`:''}`}</title><circle r={unit*.48}/><path d={`M ${-unit*.3} 0 H ${unit*.8} l ${-unit*.3} ${-unit*.2} M ${unit*.8} 0 l ${-unit*.3} ${unit*.2}`} transform={`rotate(${-spawn[2]*180/Math.PI})`}/><text y={unit*1.1} fontSize={unit*1.5} textAnchor="middle">{t('scene.spawn')} {i+1}</text></g>)}
   {world.food.map((food,i)=>{const amount=planFood(food,i,recorded,frame),label=`${food.id} · ${t(recorded?'scene.remaining':'scene.initial')}: ${amount===null?t('scene.unrecorded'):amount.toLocaleString(undefined,{maximumFractionDigits:3})}`;return <g key={food.id} className="scene-plan-food" data-food={food.id} data-amount={amount??'unrecorded'} transform={`translate(${food.position[0]} ${-food.position[1]})`}><title>{label+(world.task?.goal_food===food.id?` · ${t('scene.goal')}`:'')}</title><circle r={unit*.35} fill={amount!==null&&amount>0?'currentColor':'none'} strokeDasharray={amount===null?'3 2':undefined}/>{world.task?.goal_food===food.id&&<circle r={unit*.6} fill="none"/>}<text y={-unit*.75} fontSize={unit*1.4} textAnchor="middle">{amount===null?'?':amount.toLocaleString(undefined,{maximumFractionDigits:2})}</text></g>})}
   {flies.map((fly,slot)=>{const p=planPosition(frame,next,alpha,slot);return p&&<g key={fly.id} className="scene-plan-position" data-position-slot={slot} transform={`translate(${p[0]} ${-p[1]})`} style={{color:colors[fly.color]||colors.mint}}><title>{fly.name}</title><circle r={unit*(fly.id===selectedId ? .6 : .45)}/><text y={unit*2} fontSize={unit*1.8} textAnchor="middle">{slot+1}</text></g>})}
  </svg>
  <div className="scene-plan-legend">{flies.map((fly,slot)=><span key={fly.id}>{slot+1} · {fly.name}</span>)}<span>{t('scene.axes')}</span><span>{t(recorded?'scene.recordedNote':'scene.layoutNote')}</span>{recorded&&!world.spawns&&<span>{t('scene.spawnsUnavailable')}</span>}{flies.map((fly,slot)=>!planPosition(frame,next,alpha,slot)&&<span role="status" key={fly.id}>{fly.name} · {t('scene.positionUnavailable')}</span>)}</div>
 </div>
}
