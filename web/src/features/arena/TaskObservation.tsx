...
export function TaskObservation({match,frames,onSeek}:{match:Match;frames:Frame[];onSeek:(seconds:number)=>void}) {
  ...
  return <section className="arena-task-report panel" aria-label={zh?'任务观测结果':'Task observations'}>
    ...
    {match.result?.behavior?.map((body,slot)=>body&&<p key={slot}>{zh?'身体':'Body'} {slot+1} · {zh?'正立时间占比':'Time upright'} <strong>{(body.upright_fraction*100).toFixed(1)}%</strong>{body.first_inversion_s!=null&&<> · <button className="text-link" onClick={()=>onSeek(Math.max(0,body.first_inversion_s!-.5))}>{zh?'查看首次翻倒':'Inspect first inversion'} {body.first_inversion_s.toFixed(2)} s</button></>}</p>)}
