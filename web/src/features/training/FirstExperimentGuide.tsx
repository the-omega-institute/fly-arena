...
import type {GuideRecord} from '../guide/nextAction'
import type {ExperimentPlan} from '../arena/experimentSetup'

type Props={fly?:Fly;flies:Fly[];maps:ArenaMap[];season:Season|null;evidence?:GuideRecord|null;evidenceError?:string;budget:number;needed:number;duration:number;onCompare:(plan:ExperimentPlan)=>void;onReplay:(match:Match)=>void;onConfigure:()=>void;disabled?:boolean}
export function FirstExperimentGuide({fly,flies,maps,season,evidence=null,evidenceError,budget,needed,duration,onCompare,onReplay,onConfigure,disabled=false}:Props){
 const {t}=useI18n(),plan=guideComparisonPlan(flies,fly?.id||'',maps,season),comparison=baselineComparison(flies,fly,evidence)
 ...
 return <section className="panel first-experiment-guide" aria-label={t('training.first.title')}>
  ...
  <div className="training-pair"><div><h3>{t('training.first.compare')}</h3><p>{t(comparison?'training.first.recorded':'training.first.baseline')}</p>
   <button className="secondary" disabled={disabled||(!comparison&&!plan)} onClick={()=>{if(comparison)onReplay(comparison.match);else if(plan)onCompare(plan)}}>{t(comparison?'training.first.inspect':'training.first.prepare')}<ArrowRight size={14}/></button>
  ...
  {evidenceError&&<p role="status">{t('guide.evidenceError')} {evidenceError}</p>}
