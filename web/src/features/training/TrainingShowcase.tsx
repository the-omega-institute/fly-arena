...
export function TrainingShowcase({maps,onReplay,onBranch,revision=0}:{maps:ArenaMap[];onReplay:(m:Match)=>void;onBranch:(fly:Fly)=>void;revision?:number}){
 ...
 return <section className="evolution-showcase panel" aria-label={t('Evolution gallery')}>
  ...
  {!!runs.length&&<>
   <TrainingComparison runs={runs} maps={maps} flies={runs.flatMap(r=>r.members.map(m=>m.fly))} onOpen={id=>{setSelected(id);setGeneration(0)}} initiallyOpen/>
   ...
   {current&&<>
    ...
    <div className="showcase-individuals">{current.members.filter(m=>m.generation===generation).map(m=><article key={m.fly_id} className="training-individual">
      ...
      <button className="secondary" onClick={()=>onBranch(m.fly)}>{t('Use as starting fly')}<ArrowRight size={14}/></button>
    </article>)}</div>
   </>}
  </>}
 </section>
