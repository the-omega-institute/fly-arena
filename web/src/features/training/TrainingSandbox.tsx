...
export function TrainingSandbox({flies,identity,selected,season,maps,onLogin,onSaved,onCompete,onReplay}:Props){
  ...
  const sensoryReady=sensoryOptions.some(p=>p.id===sensoryProfile&&p.ready)
  const sensoryLabel=(id:string)=>t(sensoryOptions.find(p=>p.id===id)?.name||id)
  ...
  function restoreEnvironment(spec:{map_id:string;seed:number;duration_seconds:number;sensory_profile?:string;fitness_objective?:string;evaluation_conditions?:EvaluationCondition[]|null}){const conditions=evaluationConditions(spec);setMap(conditions[0].map_id);setSeed(conditions[0].seed);setDuration(spec.duration_seconds);setSensoryProfile(spec.sensory_profile||'odor-only-v1');setFitnessObjective(spec.fitness_objective||'food');setExtraConditions(conditions.slice(1))}
  ...
  return <div className={"training-layout "+(current?"has-session":"new-session")}>
    <aside ref={setup} className="panel training-setup">
      ...
      <label htmlFor="training-sensory-profile">{t('Sensory input profile')}<select id="training-sensory-profile" value={sensoryProfile} onChange={e=>setSensoryProfile(e.target.value)}>{sensoryOptions.map(p=><option key={p.id} value={p.id} disabled={!p.ready}>{t(p.name||p.id)}</option>)}</select></label>
      ...
      <button className="primary wide" disabled={!!busy||!profile||!sensoryReady||!fitnessReady||!!problem} onClick={start}>{busy==='create'?<Loader2 size={16} className="spin"/>:<Play size={16}/>} {t('Start training')}<ArrowRight size={16}/></button>
