...
export function TrainingSandbox({flies,identity,selected,season,maps,onLogin,onSaved,onCompete,onReplay}:Props){
  ...
  return <div className={"training-layout "+(current?"has-session":"new-session")}>
    <aside ref={setup} className="panel training-setup">
      ...
      <div className="training-pair"><label>{t('Environment')}<select value={map} onChange={e=>setMap(e.target.value)}>{maps.filter(m=>!m.task).map(m=><option key={m.id} value={m.id}>{locale==='en'?m.english:m.name}</option>)}</select></label><label>{t('Objective')}<select value={mode} onChange={e=>setMode(e.target.value as typeof mode)}><option value="forage">{t('Collect food')}</option><option value="contest">{t('Compete for food')}</option></select></label></div>
      ...
      <fieldset className="training-environments"><legend>{t('Evaluation conditions')} · {conditions.length}/4</legend>
        ...
        {extraConditions.map((condition,index)=><div key={index} className="extra-condition"><strong>{t('Condition')} {index+2}</strong><div className="training-pair">
          <label>{t('Environment')}<select value={condition.map_id} onChange={e=>setExtraConditions(old=>old.map((c,i)=>i===index?{...c,map_id:e.target.value}:c))}>{maps.filter(m=>!m.task).map(m=><option key={m.id} value={m.id}>{locale==='en'?m.english:m.name}</option>)}</select></label>
