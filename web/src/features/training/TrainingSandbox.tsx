...
export function TrainingSandbox({flies,identity,selected,season,maps,onLogin,onSaved,onCompete,onReplay}:Props){
  ...
  async function save(member:Member,next:'library'|'compete'|'parent'|'branch'='library'){if(current)await act(member.fly_id,async()=>{
    ...
    if(next==='compete'||next==='parent')onCompete(fly,{map_id:current.spec.map_id,seed:current.spec.seed,duration_seconds:current.spec.duration_seconds,opponent_id:current.spec.mode==='contest'?current.spec.opponent_id:null},next==='parent'?current.members.find(parent=>parent.fly_id===member.fly.spec.parent_id)?.fly:undefined)
