...
export function TrainingSandbox({flies,identity,selected,season,maps,onLogin,onSaved,onCompete,onReplay}:Props){
  ...
  function restoreEnvironment(spec:{map_id:string;seed:number;duration_seconds:number;sensory_profile?:string;evaluation_conditions?:EvaluationCondition[]|null}){const conditions=evaluationConditions(spec);setMap(conditions[0].map_id);setSeed(conditions[0].seed);setDuration(spec.duration_seconds);setSensoryProfile(spec.sensory_profile||'odor-only-v1');setExtraConditions(conditions.slice(1))}
  ...
  return <div className={"training-layout "+(current?"has-session":"new-session")}>
    <aside ref={setup} className="panel training-setup">
      ...
      <button className="primary wide" disabled={!!busy||!profile||!sensoryReady||!!problem} onClick={start}>{busy==='create'?<Loader2 size={16} className="spin"/>:<Play size={16}/>} {t('Start training')}<ArrowRight size={16}/></button>
