...
import {shortHorizonTrainingGuidance} from './planGuidance'
...
type Props={matches?:Match[];onCompareWT?:(plan:ExperimentPlan)=>void;flies:Fly[];identity:Identity|null;selected:string;season:Season|null;maps:ArenaMap[];onLogin:()=>void;onSaved:(fly:Fly)=>Promise<void>;onCompete:(fly:Fly,setup?:CompetitionSetup,reference?:Fly)=>void;onReplay:(match:Match)=>void}
...
export function TrainingSandbox({onCompareWT,matches=[],flies,identity,selected,season,maps,onLogin,onSaved,onCompete,onReplay}:Props){
  ...
  const [showcaseOpen,setShowcaseOpen]=useState(()=>new URLSearchParams(location.hash.slice(1)).has('showcase'))
  ...
  const horizonGuidance=shortHorizonTrainingGuidance({duration,fitnessObjective})
  ...
  return <div className={"training-layout "+(current?"has-session":"new-session")}>
    ...
    <aside ref={setup} className="panel training-setup">
      ...
      {horizonGuidance&&<p className="training-hint training-warning">{t(horizonGuidance)}</p>}
