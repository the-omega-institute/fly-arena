...
export function planProblem(p:PlanLimits):string|null{
  ...
  if(p.conditions){
    ...
    if(p.conditions.some(c=>!['orchard','maze','scarcity','ring','terrarium','enclosure'].includes(c.map_id)||!Number.isInteger(c.seed)||c.seed<0||c.seed>2147483647))return 'Use a valid map and whole-number seed for every condition.'
