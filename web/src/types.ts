...
export type BodyModel = {meshes:Record<string,{vertices:number[];faces:number[]}>;geoms:{id:number;slot:number;name:string;mesh:string}[];food_geoms?:{id:number;food_id:string}[]}
...
export type SensorySample = {odor:number[];visual:number[];visual_status?:string;touch:number;touch_status?:string;nearest_food:number|null;mouth_distance:number|null}
