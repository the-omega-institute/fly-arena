...
export type Season = {id:string;name:string;connectome:{sha256:string;neuron_count:number;edge_count:number;synaptic_contacts:number;circuits:Circuit[]};budget:{points:number};invite_required:boolean;runtime_sha256:string;default_bridge_profile?:string;match_profiles?:MatchProfile[];sensory_profiles?:SensoryProfile[]}
...
export type ReplayParticipant = Pick<Fly,'id'|'name'|'color'|'artifact_id'|'spec'> & {report?:Pick<Report,'budget_used'|'budget_limit'>}
export type Match = {participants?:ReplayParticipant[];tournament?:string|null;id:string;status:string;progress:number;attempt:number;created:number;error:string|null;request:{fly_ids:string[];map_id:string;mode:string;seed:number;duration_seconds:number;bridge_profile?:string;sensory_profile?:string};result:{winner_slot:number|null;scores:number[];outcome:string;receipt_sha256:string}|null}
...
export type ReplayEvent = {type:string;tick:number;slot?:number;food?:number;amount?:number;values?:number[];mouth_distance?:number;slots?:number[];objects?:string[]}
export type BrainSample = {circuits:Record<string,number>;top_nodes?:{id:string;activity:number}[];sampled_nodes?:{id:string;activity:number}[];sampling?:{kind:string;count:number}}
export type SensorySample = {odor:number[];visual:number[];visual_status?:string;touch:number;touch_status?:string;sensory_profile?:string;contact_food?:string[];contact_environment?:string[];tactile_activity?:number|null;nearest_food:number|null;mouth_distance:number|null}
