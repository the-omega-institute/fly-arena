...
export type Season = {id:string;name:string;connectome:{sha256:string;neuron_count:number;edge_count:number;synaptic_contacts:number;circuits:Circuit[]};budget:{points:number};invite_required:boolean;runtime_sha256:string;default_bridge_profile?:string;match_profiles?:MatchProfile[];sensory_profiles?:SensoryProfile[];training_sensory_profiles?:SensoryProfile[];gallery_copy_available?:boolean}
...
export type NeuralGraph = {neurons:{id:string;type?:string|null;class?:string|null;side?:string|null;nt?:string|null}[];edges:{pre:string;post:string;edge:number;count:number;baseline_weight?:number;weight?:number;multiplier?:number}[];anchors?:string[]}
export type BrainDesignGraph = {schema:'brain-neighborhood/v1';artifact_id:string;connectome_sha256:string;weights_sha256:string;selection:string;circuits:Record<string,NeuralGraph>}
