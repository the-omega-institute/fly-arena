...
export type GeometryQuaternionWxyz = [number,number,number,number]
export type ArenaGeometryContract = {id:'arena-geometry-v1';units:'mm';worldAxes:{x:'east';y:'north';z:'up'};quaternionOrder:'wxyz'}
...
export type Scene = {task?:ArenaTask;replay_policy?:{pose_ticks:number};body:BodyModel;size:number;obstacles:ArenaObstacle[];food:{position:number[];initial:number;id:string}[];flies:{id:string;name:string;color:string}[];ring_radius?:number;habitat?:string}
...
export const modes:Record<string,string> = {forage:'单蝇觅食',contest:'双蝇抢食',sumo:'擂台争夺',duel:'接触对抗 / 领地争夺'}
