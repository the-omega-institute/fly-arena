...
export default function App(){
  ...
  async function previewExample(){await action('neural-example',async()=>{const response=await fetch('/examples/neural-preview-nectar-v2.json');if(!response.ok)throw Error(locale==='zh-CN'?'刺激样本暂不可用':'Stimulus example is unavailable');const data:NeuralPreview=await response.json();if(data.schema!=='neural-design-preview/v2'||!data.example)throw Error('Invalid stimulus example');setNeuralPreview({data,specKey:null})})}
