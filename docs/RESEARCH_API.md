...
The 202 response is an experiment with `id`, `owner`, `status`, `error`, `spec`, `subjects`, `reports`, `comparison`, `created`, and diagnostic generation/condition fields. `subjects` is the server-frozen ordered wildtype/official/design triplet; each entry contains `role`, `fly_id`, `name`, and `artifact_id`. `GET /experiments` lists the latest 100 public experiments; `GET /experiments/{id}` returns one or 404. Published research designs and reports have the same public workspace visibility as the existing MVP.
...
`reference_kind` is server-owned: `wildtype`, `official`, `user` or `ai`. `release_id` identifies the canonical server seed release, otherwise null. `submission_channel` is `seed`, `web` or `api`. The deterministic registry derives references from trusted seed definitions, graph/profile and validated compiled artifacts. It does not migrate an existing record based on name or `owner="arena"`; legacy records remain unprivileged unless validated through the trusted registry path. Same-name users and cloned official weights remain user records.
