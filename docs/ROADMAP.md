...
2. **下一张地图先做一个真正立体的场景。** 腐果、低坡、落叶桥、窄道与多个资源点；实际碰撞和可视几何一致，配合跟随/自由镜头与感觉场解释。现有场景仍主要是平地，不能把已有三维身体回放等同于完成复杂三维环境。
3. **用户可选大脑运行方式与学习算法。** 当前仍是固定 LIF、基础进化/随机搜索/外部参数提案。先加入成熟的低维进化优化器，再实现连接结构约束下的可微分发放率模型和实际训练器；根据逐步接口与算力实测接入强化学习。比赛内可塑性和脉冲替代梯度明确列为后续能力，不放未实现按钮。
4. **社区竞争和演化。** 完成作者/谱系/训练方法的发现、挑战他人、赛后差异说明和从公开后代继续分叉。开放比赛允许不同模型，科学比较明确显示模型和训练条件差异。
