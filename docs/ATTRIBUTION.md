...
The project-authored code is MIT-licensed; third-party data, assets, fonts and
dependencies have separate terms documented in [NOTICE](../NOTICE.md). This
file records source and scope details; it is not a license grant for upstream
materials.
...
- FlyGym 2.1.0 / NeuroMechFly body, locomotion demonstration and MuJoCo integration: https://github.com/NeLy-EPFL/flygym . FlyGym source is documented upstream as Apache-2.0; the specific packaged models, meshes and downloaded assets retain upstream terms that must be checked separately. The preview uses its simplified anatomical model. The fixed low-level gait is an upstream controller, not a learned fruit fly motor system.
- MuJoCo: https://github.com/google-deepmind/mujoco . Native physics implementation under Apache-2.0, locked with dependencies in `uv.lock`.
...
- DM Sans and Instrument Serif are loaded from Google Fonts under SIL OFL 1.1. The tracked `web/public/examples/*.png` files are project-generated simulation renders. See [NOTICE](../NOTICE.md) for the separate terms and redistribution boundaries.
