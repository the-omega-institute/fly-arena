...
**当前状态（2026-09-20 更新）**：沿用已有 active Goal。本文已落盘，产品目标尚未完成。单蝇30秒觅食、同条件WT对照、两场换位的30秒真实双蝇竞争及参赛脑图已有本地公开回放；回放可以生成自己的后代草稿并导出给AI。身体跟随镜头已通过前端检查并发布到现有本地8080服务：切换观察对象同步切换镜头与脑图，拖动时间保持视角和缩放，返回全景恢复场景观察。新后端、远端部署、浏览器视觉验收和完整在线设计—训练—后代竞技仍待完成。

**已有成果与限制**：Nectar单蝇30秒摄取14.3608，18.46秒再次接触另一处食物，全程未记录倒置；WT同条件摄取0.508，12.30秒倒置。随后两场同场竞争仅交换出生位置，Nectar/WT摄取分别为20/0和10.4504/0.0688；第二场双方实际消耗同一份食物。但两场所有参赛者最终都倒置，10秒后均无摄取，因此持续身体控制与竞争仍未达标。共享动作读出此前用Nectar校准，单种子结果不能证明普遍优势或演化成功。上述计算实际在本机arm64 CPU运行，不能称为4060计算或渲染。

**代码交接**：后续源码使用 `/private/tmp/fly-arena-release-candidate`，分支 `feat/neural-life-theater`；已汇总此前增量和跟随镜头，已保存本地提交，尚未推送。本地后端CI结果为500通过、11跳过、3排除；77项前端检查、31项DOM流程检查与生产构建全部通过。页面、静态资源、两场各3001帧及双方脑图已HTTP核对；不等于浏览器视觉验收。完整Goal不以这些检查通过作为完成依据。

**当前新增工作：按持续行为选择后代（2026-09-20）**：训练新增可选 `sustained-foraging-v1`，适应度为（总摄取 + 后半程摄取）× 未倒置时间比例。摄取来自事件，姿态来自实际胸部记录，时间按采样间隔计；这是一项明确的工程目标，不改变比赛胜负，也不证明已经学会行为。网页和AI脚本共用该目标，策略比较会标明目标不同。四份已有30秒回放已增加评估明细并发布；这些是旧记录的重新评估，不是新演化结果。新后端尚未部署，在线创建入口按服务能力限制。已准备WT的2代×2个体×10秒CEM实验，等待当前CI结束后串行执行，实际执行位置为本机CPU；结果未出，不预设改善。
...
1. **完成当前增量的版本交付**：前端检查、用户流程检查、构建与本地静态发布已完成。保存整合后的本地提交与恢复补丁，推进正常远端CI和合并；跟随镜头的浏览器视觉验收仍待完成。身体、镜头、记分牌与脑图观察同一只果蝇，暂停和拖动时间轴同步。
...
最新证据位于 `var/research/enclosed-duel/comparison.json`、`var/research/enclosed-duel/http-publication.json` 和 `var/research/release-candidate/backend-tests.log`。后文保留历史记录，旧版本状态以上述最新接续点为准。
