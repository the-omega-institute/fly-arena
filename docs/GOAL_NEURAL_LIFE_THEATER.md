...
**本轮界面增量：亲代与后代的行为对照**：演化画廊新增跨代个体选择，同屏列出实验基线、记录中的亲代和所选后代的总摄取、后半程摄取、未倒置时间及首次倒置。每一行打开对应身体与脑活动回放；对照按地图、种子、时长、感觉/动力学配置、出生位与对手匹配，缺失记录不补零。可展开查看三者的回路倍率、神经参数及逐边修改数量。前端78项检查、34项DOM交互检查和构建通过；本轮合并与静态发布状态另行记录，浏览器视觉验收仍待完成。
