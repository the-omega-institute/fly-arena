...
**本轮可观看增量：两份生命记录同步回放**：竞技场新增“同步对照另一份生命记录”。两个真实三维身体、各自的感觉输入与可展开脑图共用播放时钟；点击任意侧事件可同步跳转，脑区与局部神经元采用共同固定色标。保留各自地图、种子和模型说明，不把不同条件说成相同实验；只播放共同记录区间，身体按记录姿态插值，神经活动保持实际采样。切换回放会清空旧数据，关闭原单份舞台以避免同时渲染三个场景。相关前端78项检查和35项交互检查通过；版本交付状态见本轮交接记录，浏览器视觉验收仍未完成。
