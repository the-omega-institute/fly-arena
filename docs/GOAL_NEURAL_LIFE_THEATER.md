...
**普通用户回放的大脑设计绑定**：流程核对发现，普通比赛接口原来没有参赛者设计快照，精选回放的实际权重图依赖手工打包。当前增量为普通比赛列表/详情返回与参赛artifact一致的公开设计快照，并提供按参赛位置读取的有界连接邻域；复用既有打包算法和编译权重，邻居没有采样时仍不伪造活动。网页按回放身份加载实际权重；旧服务或不匹配身份只显示基础结构并明确标出权重不可用。49项相关后端检查、36项交互检查通过；远端CI与交付继续推进。另有一个完整连接组API流程实验在独立库中运行：设计、神经预览、两代短训练、保存后代、与WT比赛。该实验不启动监听服务，不等于在线部署或浏览器验收。
