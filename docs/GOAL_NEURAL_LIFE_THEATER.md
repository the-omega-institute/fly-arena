...
在真实 MuJoCo 几何中复现：将果蝇身体抬高 0.8 mm，口器参考点仍在食物的 1.1 mm 平面范围、2.5 mm 高度阈值内，但口器探针已不接触食物。旧距离规则仍允许计分，这会使地形和高度产生不真实的摄取结果。

新规则 `arena-ground-contact-v2` 要求每个物理 tick 同时满足原距离/高度限制与实际 MuJoCo 口器—食物接触，再按原速率与共享容量分配资源。接触探针仍是明确标注的工程化摄食传感器，不代表已仿真出真实可伸缩口器。规则版本和接触要求记录在 runtime；网页回放来源面板区分“必须实际接触食物”和“旧版距离与高度条件”。

此前公开的 1 秒单体样本，以及正在运行/排队的封闭果园对照，都保留旧摄取规则。它们包含实际接触观测与旧规则计分，不应描述成新接触规则下的摄取成绩。原始回放与成绩不改写。
