"""Registered v15 normal allocation with fresh unchanged v14 control."""
...
from .cadence_v14 import ClearanceHybridController, ClearanceObservation
from .cadence_v15 import AllocationHybridController, AllocationObservation, PROFILE, CONTROL
...
APPROVAL = ORIGINAL / "var/sshx/behavior-v15/approved-plan.json"
...
BASE = "0682e3d9a31336769f2b131cfb4e5aa2a3c409d2"
...
class AllocationSensor(WholeFootSensor):
    ...
    def observation(self,native,qpos,tick,heights,geoms,vertices):
        ...
        return AllocationObservation(native,heights,geoms,vertices,np.asarray(qpos)[self.qadr].copy(),
...
def new_body(seed, profile):
    ...
    cls = {PROFILE: AllocationHybridController, CONTROL: ClearanceHybridController}[profile]
    ...
    body.clearance_sensor = AllocationSensor(body.model,controller) if profile==PROFILE else WholeFootSensor(body.model)
    if profile==PROFILE:
...
class Budget:
    ...
    def check(self):
        ...
        if (self.steps > 2600000 or report['wall_seconds'] > 14400
            ...
            raise RuntimeError('registered v15 resource cap exceeded')
    def report(self):
        ...
        return {'mechanical_seconds': self.steps*DT, 'full_network_seconds': 0,
                ...
                'evidence_bytes': sum(p.stat().st_size for base in (self.root,Path('/tmp/fly-v15-scratch')) for p in base.rglob('*') if p.is_file()), 'workers': 1}
...
def _controller_schema(controller):
    ...
    for name in ("release_start", "release_end", "last_clearance", "last_trigger", "last_minimum_geom", "last_minimum_vertex", "last_allocation", "last_allocation_diagnostics", "last_native_template"):
...
def freeze(root):
    ...
    sources = {}
    paths = source_files() + [
        ROOT / "pyproject.toml", ROOT / "uv.lock", ROOT / "scripts/report_behavior_v15.py", ROOT / "tests/test_behavior_v15.py", ROOT / "tests/test_behavior_v12_correction.py", GOLDEN,
    ]
    ...
    swing_periods = {leg: [float(c.preprogrammed_steps.swing_period[leg][0]),
                     ...
                     for leg in c.legs}
    registration = {
        "schema": "mechanical-registration/v15", "candidate": PROFILE,
        ...
        "controller": {
            "candidate_equation": "v14 trigger/scalar/CPG unchanged; updated strict swing qcmd=q0+min(dreq,b,dlim)*N*N.T*Jz/||N*N.T*Jz||; zero retraction offset in stance",
            "only_change_from_v14": "replace additive retraction realization by exact strict-swing minimum-point normal allocation; no retraction stance offset; native stumbling unchanged",
            ...
            "net_correction_semantics": "v14 scalar*gain; candidate only unchanged stumbling scalar*gain, retraction vector stored separately as allocation",
            "allocation_diagnostics": ["h0_mm","authority_mm_per_rad","requested_rad","budget_rad","range_limit_rad_minus1_if_unbounded","residual_mm","tangent_rank","limit_flags_budget1_range2"],
        ...
        "panel": {
            "development": "unchanged v14+candidate x seeds42/43 x8 =128s",
...
def step(body, u, budget=None):
    ...
    if body.clearance_sensor is not None and float(np.mean(u))>1e-4:
        ...
        else:observation=ClearanceObservation(observation,heights,geoms,vertices)
...
def _binding(root, reg):
    return {
        ...
        "generation": "v15-development-seed42-straight02-active-tick10000",
    }
...
def run(root, anchor):
    ...
    try:
        ...
        from .analyze_v15 import analyze_panel
        from .verify_v15 import verify_panel
        ...
        if decision['candidate_passed']:
            ...
            from .verify_v15 import verify_repeat
            verify_repeat(root)
            ...
            from .verify_v15 import verify_restoration
