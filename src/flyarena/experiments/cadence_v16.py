...
PROFILE = 'budget-preserving-tangent-relaxation-v16'
CONTROL = 'minimum-vertex-normal-retraction-v15'
STATE_VERSION = 'cadence-controller-state/v16'
OBSERVATION_VERSION = 'whole-foot-observation/v16'
...
def normal_allocation(J, h, q, q0, lower, upper, budget):
    """Registered two-direction law; diagnostics are native eight plus five.

    Extra fields: branch(0 unchanged,1 relaxed,2 maximum authority), x/y(rad)
    before range scaling, c(mm/rad), and final ray scale lambda(unitless).
    The original eight retain h0,a,D/a,b,ray limit(rad),residual,rank,flags.
    Flags: remaining normal-budget infeasibility=1, active range clipping=2.
    """
    if (np.shape(J)!=(3,7) or any(np.shape(v)!=(7,) for v in (q,q0,lower,upper))
            or not all(np.isfinite(x).all() for x in (J,q,q0))
            or not np.isfinite([h,budget]).all() or budget<0
            or np.isnan(lower).any() or np.isnan(upper).any()
            or np.isposinf(lower).any() or np.isneginf(upper).any()
            or np.any(lower>upper) or np.any(q0<lower) or np.any(q0>upper)):
    ...
    if authority<=tau:
        if h0<.05:raise ValueError('infeasible normal realization: rank/authority')
        return np.zeros(7),np.array([h0,authority,0.,budget,-1.,0.,rank,0.]),np.array([0.,0.,0.,0.,1.])
    n=w/authority;r=J[2]-w;c=float(np.linalg.norm(r));D=max(0.,.05-h0)
    requested=D/authority
    if not np.isfinite([c,D,requested]).all():raise ValueError('nonfinite task decomposition')
    if D<=authority*budget or c<=tau:
        # Keep accepted v15 feasible/degenerate arithmetic bitwise identical.
        limits=np.full(7,np.inf);positive=n>0;negative=n<0
        limits[positive]=(upper[positive]-q0[positive])/n[positive]
        limits[negative]=(lower[negative]-q0[negative])/n[negative]
        ...
        nominal=min(requested,budget);scale=distance/nominal if nominal else 1.
        return offset,np.array([h0,authority,requested,budget,limit if np.isfinite(limit) else -1.,residual,rank,flags]),np.array([0.,nominal,0.,c,scale])
    m=r/c;L=float(np.hypot(authority,c))
    if D<L*budget:
        u=float(np.sqrt(max(0.,(L*budget)**2-D**2)))
        x=(authority*D+c*u)/(L*L);y=max(0.,(c*D-authority*u)/(L*L));branch=1
    else:
        x=budget*authority/L;y=budget*c/L;branch=2
    offset=x*n+y*m;length=float(np.linalg.norm(offset))
    if not np.isfinite(offset).all() or not np.isfinite([L,x,y,length]).all():
        raise ValueError('nonfinite relaxed allocation')
    if length>budget:
        shrink=budget/length;offset*=shrink;x*=shrink;y*=shrink;length=float(np.linalg.norm(offset))
    ratios=np.full(7,np.inf);positive=offset>0;negative=offset<0
    ratios[positive]=(upper[positive]-q0[positive])/offset[positive]
    ratios[negative]=(lower[negative]-q0[negative])/offset[negative]
    maxscale=float(ratios.min());scale=min(1.,maxscale)
    limit=length*maxscale if length and np.isfinite(maxscale) else -1.
    offset*=scale
    residual=max(0.,.05-float(h0+J[2]@offset))
    flags=int(D>L*budget)+2*int(scale<1.)
    diagnostics=np.array([h0,authority,requested,budget,limit,residual,rank,flags])
    return offset,diagnostics,np.array([branch,x,y,c,scale])
...
class AllocationHybridController(ExcursionHybridController):
    def __post_init__(self):
        ...
        if (not np.array_equal(self.release_start,np.zeros(6)) or np.any(self.release_end<=0)
            ...
            raise ValueError('native source constants differ from approved v16 contract')

    def reset(self, **kwargs):
        super().reset(**kwargs)
        self.last_allocation=np.zeros((6,7))
        self.last_allocation_diagnostics=np.zeros((6,8))
        self.last_relaxation_diagnostics=np.zeros((6,5))
    ...
    def step(self, descending_signal, obs):
        ...
        if u.shape!=(2,) or not np.isfinite(u).all() or np.any(u<0):
            raise ValueError('v16 requires two finite nonnegative v4 outputs')
        ...
        if not isinstance(obs,AllocationObservation):
            raise ValueError('v16 requires a versioned whole-foot observation')
        ...
        if not all(np.isfinite(x).all() for x in (action.joint_angles,shadow.last_allocation,shadow.last_allocation_diagnostics,shadow.last_relaxation_diagnostics,shadow.last_native_template)):
        ...
        for index,leg in enumerate(self.legs):
            ...
            if self.retraction_correction[index]>0:
                ...
                if self.release_start[index]<wrapped_phase<self.release_end[index]:
                    ...
                    offset,diagnostic,relaxation=normal_allocation(obs.jacobian[index],obs.clearance[index],
                    ...
                    self.last_relaxation_diagnostics[index]=relaxation
    ...
    def restore(self,checkpoint):
        if set(checkpoint)!={'version','state'} or checkpoint['version']!=STATE_VERSION or set(checkpoint['state'])!=set(self.__dict__):
            raise ValueError('unsupported/incomplete v16 checkpoint')
