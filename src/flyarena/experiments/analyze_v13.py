"""Prospective v13 analysis using accepted strict paired-state and cycle contracts."""
import json
...
def trial_identity(root, trial, registration, expected):
    ...
    if any(value != wanted for value in starts) or not json.loads((trial/'trial-terminal.json').read_text())['complete']:
        raise ValueError('v13 retained trial identity/completion mismatch')
    return starts[0]
...
def analyze_panel(root, stage, budget):
    ...
    if actual != names:
        raise ValueError('v13 panel exact trial set mismatch')
    ...
    result = {'schema':'behavior-v13-analysis/v1','stage':stage,'registration_sha256':file_sha(root/'registration.json'),
