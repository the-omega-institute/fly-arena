"""Prospective v16 analysis using accepted strict paired-state and cycle contracts."""
import json
...
def trial_identity(root, trial, registration, expected):
    ...
    if any(value != wanted for value in starts) or not json.loads((trial/'trial-terminal.json').read_text())['complete']:
        raise ValueError('v16 retained trial identity/completion mismatch')
    return starts[0]
...
def analyze_trial(
    ...
    return {
        ...
        "relaxation": {
            "branches_active_leg_rows": [int(np.count_nonzero(core[3001:25001,cs['relaxation_diagnostics']].reshape(-1,6,5)[:,:,0]==branch)) for branch in (0,1,2)],
            "rowspace_component_max_rad": core[3001:25001,cs['relaxation_diagnostics']].reshape(-1,6,5)[:,:,2].max(axis=0).tolist(),
            "positive_local_residual_leg_rows": (core[3001:25001,cs['allocation_diagnostics']].reshape(-1,6,8)[:,:,5]>0).sum(axis=0).tolist(),
            "minimum_geom_or_vertex_changes": np.count_nonzero((np.diff(core[3001:25001,cs['minimum_vertex']],axis=0)!=0)|(np.diff(core[3001:25001,cs['minimum_geom']],axis=0)!=0),axis=0).tolist(),
            "inactive_branch_zero_note": "branch0 includes held/stance zero diagnostics; branch1/2 identify actual relaxation only"},
        "schema": "behavior-v16-trial/v1",
        "conditions": meta,
        "state_contract": {
...
def analyze_panel(root, stage, budget):
    ...
    if actual != names:
        raise ValueError('v16 panel exact trial set mismatch')
    ...
    result = {'schema':'behavior-v16-analysis/v1','stage':stage,'registration_sha256':file_sha(root/'registration.json'),
