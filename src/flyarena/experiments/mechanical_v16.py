"""Registered v16 two-direction relaxation with fresh unchanged v15 control."""
...
from .cadence_v15 import AllocationHybridController as V15Controller, AllocationObservation as V15Observation
from .cadence_v16 import AllocationHybridController, AllocationObservation, PROFILE, CONTROL
from flygym_demo.complex_terrain.common import LocomotionAction
from .evidence_v10 import NumericEvidence
from .mechanical_v13 import _sha_array, _numeric_cache, _compiled_arrays
...
APPROVAL = ORIGINAL / "var/sshx/behavior-v16/approved-plan.json"
SUBJECTS = ORIGINAL / "var/sshx/sensorimotor-v7/subjects.json"
GOLDEN = Path("/tmp/fly-arena-behavior-v14/var/behavior-v14/mechanical-01/development/whole-foot-retraction-feedback-v14--42--straight-02/core/chunk-00003.npz")
BASE = "d84169e547ffad2416c455bf1699ecd53f91a4bd"
...
CORE = [
    ...
    ("sensed_clearance", 6), ("trigger_mask", 6), ("minimum_geom", 6), ("minimum_vertex", 6),
    ("native_template", 42), ("native_observation", 64),
    ("allocation",42), ("allocation_diagnostics",48), ("observation_tick",1),
    ("relaxation_diagnostics",30),
]
DENSE = [
...
def correction_matrix(controller):
    order=controller.output_dof_order if controller.output_dof_order is not None else get_default_locomotion_dof_order()
    matrix=np.zeros((42,6))
    for index,leg in enumerate(controller.legs):
        vector=_CORRECTION_VECTORS[leg[1]]
        if leg.startswith('r'):vector=vector*_RIGHT_LEG_CORRECTION_SIGN
        for j,spec in enumerate(controller.preprogrammed_steps.dofs_per_leg):
            matrix[order.index(dof_spec_to_jointdof(leg,spec)),index]=vector[j]
    return matrix


class WholeFootSensor:
    """Read-only actual-state sensor with a reusable detached kinematics workspace."""
    def __init__(self, model):
        self.model=model;self.data=mj.MjData(model);self.foot=foot_ids(model)
        self.vertices=[]
        for geom in self.foot:
            mesh=int(model.geom_dataid[geom]);a=int(model.mesh_vertadr[mesh]);n=int(model.mesh_vertnum[mesh])
            self.vertices.append(np.asarray(model.mesh_vert[a:a+n],dtype=np.float64))
        ground=mj.mj_name2id(model,mj.mjtObj.mjOBJ_GEOM,'ground_plane')
        if (ground<0 or model.geom_type[ground]!=mj.mjtGeom.mjGEOM_PLANE
                or not np.array_equal(model.geom_pos[ground],np.zeros(3))
                or not np.array_equal(model.geom_quat[ground],np.array([1.,0.,0.,0.]))):
            raise ValueError('sensor requires registered flat local plane z=0')

    def read(self,qpos):
        qpos=np.asarray(qpos)
        if qpos.shape!=(self.model.nq,) or not np.isfinite(qpos).all():
            raise ValueError('invalid sensor qpos')
        self.data.qpos[:]=qpos
        mj.mj_kinematics(self.model,self.data)
        heights=np.full(6,np.inf);geoms=np.full(6,-1,dtype=np.int64);indices=np.full(6,-1,dtype=np.int64)
        for index,(geom,vertices) in enumerate(zip(self.foot,self.vertices)):
            z=vertices@self.data.geom_xmat[geom].reshape(3,3)[2]+self.data.geom_xpos[geom,2]
            vertex=int(np.argmin(z));height=float(z[vertex]);leg=index//5
            if height<heights[leg] or (height==heights[leg] and (int(geom),vertex)<(geoms[leg],indices[leg])):
                heights[leg]=height;geoms[leg]=geom;indices[leg]=vertex
        if not np.isfinite(heights).all():raise ValueError('nonfinite sensed clearance')
        return heights,geoms,indices


class AllocationSensor(WholeFootSensor):
    def __init__(self,model,controller):
        super().__init__(model)
        self.observation_type=AllocationObservation if isinstance(controller,AllocationHybridController) else V15Observation
        order=get_default_locomotion_dof_order()
        self.indices=np.array([[order.index(dof_spec_to_jointdof(leg,spec))
            for spec in controller.preprogrammed_steps.dofs_per_leg] for leg in controller.legs])
        joints=model.actuator_trnid[:42,0]
        if len(set(joints.tolist()))!=42 or np.any(model.jnt_type[joints]!=mj.mjtJoint.mjJNT_HINGE):
            raise ValueError('missing/non-hinge active joint map')
        self.qadr=model.jnt_qposadr[joints][self.indices]
        self.vadr=model.jnt_dofadr[joints][self.indices]
        lower=np.full(42,-np.inf);upper=np.full(42,np.inf)
        for i,joint in enumerate(joints):
            for enabled,interval in ((model.jnt_limited[joint],model.jnt_range[joint]),
                                     (model.actuator_ctrllimited[i],model.actuator_ctrlrange[i])):
                if enabled:
                    if not np.isfinite(interval).all() or interval[0]>interval[1]:raise ValueError('invalid enabled range')
                    lower[i]=max(lower[i],interval[0]);upper[i]=min(upper[i],interval[1])
        self.lower=lower[self.indices];self.upper=upper[self.indices]
        if np.any(self.lower>self.upper):raise ValueError('empty compiled interval')
        arrays=_compiled_arrays(model)
        arrays.update({name:np.asarray(getattr(model,name)) for name in ('mesh_vert','mesh_vertadr','mesh_vertnum','geom_dataid','geom_bodyid')})
        self.model_hash=digest({name:_sha_array(value) for name,value in arrays.items()})

    def observation(self,native,qpos,tick,heights,geoms,vertices):
        # read() already updated this detached qpos/kinematics workspace.
        mj.mj_comPos(self.model,self.data)
        jacobian=np.empty((6,3,7));jacp=np.zeros((3,self.model.nv))
        for leg,(geom,vertex) in enumerate(zip(geoms,vertices)):
            mesh=self.model.geom_dataid[geom]
            local=self.model.mesh_vert[self.model.mesh_vertadr[mesh]+vertex].astype(np.float64)
            point=self.data.geom_xpos[geom]+self.data.geom_xmat[geom].reshape(3,3)@local
            mj.mj_jac(self.model,self.data,jacp,None,point,int(self.model.geom_bodyid[geom]))
            jacobian[leg]=jacp[:,self.vadr[leg]]
        return self.observation_type(native,heights,geoms,vertices,np.asarray(qpos)[self.qadr].copy(),
            jacobian,self.lower.copy(),self.upper.copy(),int(tick),self.model_hash)


def new_body(seed, profile):
    body = Bodies(deepcopy(SCENE), 1, seed)
    cls = {PROFILE: AllocationHybridController, CONTROL: V15Controller}[profile]
    ...
    body.clearance_sensor = AllocationSensor(body.model,controller)
    if profile in (CONTROL,PROFILE):
        controller.model_hash=body.clearance_sensor.model_hash
        controller.compiled_lower=body.clearance_sensor.lower.copy()
        controller.compiled_upper=body.clearance_sensor.upper.copy()
    return body
...
class Budget:
    ...
    def check(self):
        ...
        if (self.steps > 2600000 or report['wall_seconds'] > 14400
                ...
                or report['filesystem_free_bytes'] < 20*1024**3):
            raise RuntimeError('registered v16 resource cap exceeded')
    def report(self):
        ...
        return {'mechanical_seconds': self.steps*DT, 'full_network_seconds': 0,
                ...
                'evidence_bytes': sum(p.stat().st_size for base in (self.root,Path('/tmp/fly-v16-scratch')) for p in base.rglob('*') if p.is_file()), 'workers': 1, 'filesystem_free_bytes': shutil.disk_usage(self.root).free}
...
def _controller_schema(controller):
    ...
    for name in ("release_start", "release_end", "last_clearance", "last_trigger", "last_minimum_geom", "last_minimum_vertex", "last_allocation", "last_allocation_diagnostics", "last_relaxation_diagnostics", "last_native_template"):
        if hasattr(controller, name):
            arrays[name] = getattr(controller, name)
    result = {}
...
def _controller_state_manifest(controller):
    ...
    # Include all new immutable bindings and mutable diagnostics in the digest.
    for name in ('compiled_lower','compiled_upper'):
        if hasattr(controller,name):scalars[name+'_sha256']=_sha_array(getattr(controller,name))
    for name in ('last_observation_tick','model_hash','observation_version'):
        if hasattr(controller,name):scalars[name]=getattr(controller,name)
...
def freeze(root):
    if shutil.disk_usage(root.parent if root.parent.exists() else ROOT).free<20*1024**3:
        raise RuntimeError("filesystem free floor before registration")
    root.mkdir(parents=True, exist_ok=False)
    shutil.copyfile(APPROVAL, root / "approved-plan.json")
    shutil.copyfile(SUBJECTS, root / "subjects.json")
    sources = {}
    paths = source_files() + [
        ROOT / "pyproject.toml", ROOT / "uv.lock", ROOT / "scripts/report_behavior_v16.py", ROOT / "tests/test_behavior_v16.py", ROOT / "tests/test_behavior_v12_correction.py", GOLDEN,
    ]
    ...
    swing_periods = {leg: [float(c.preprogrammed_steps.swing_period[leg][0]),
                     ...
                     for leg in c.legs}
    registration = {
        "schema": "mechanical-registration/v16", "candidate": PROFILE,
        ...
        "controller": {
            "candidate_equation": "v15 feasible allocation unchanged; only D>a*b permits exact two-direction minimal-y relaxation within b; zero stance retraction",
            "only_change_from_v15": "exact two-direction tangential relaxation only when original local normal task exceeds inherited norm budget",
            "allocation": json.loads(APPROVAL.read_text())["selected_conclusion"]["exact_law"],
            "observation_model_hash": body.clearance_sensor.model_hash,
            "net_correction_semantics": "both profiles unchanged stumbling scalar*gain only; retraction offset stored separately as allocation",
            "allocation_diagnostics": ["h0_mm","authority_mm_per_rad","requested_rad","budget_rad","ray_limit_rad_minus1_if_unbounded","actual_local_residual_mm","tangent_rank","limit_flags_normalbudget1_range2"],
            "relaxation_diagnostics": ["branch_0unchanged_1relaxed_2maxauthority","x_rad_before_ray_scale","y_rad_before_ray_scale","rowspace_authority_mm_per_rad","ray_scale_lambda"],
            "allocation_joint_map": body.clearance_sensor.indices.tolist(),
            "allocation_lower": [[None if np.isneginf(x) else float(x) for x in row] for row in body.clearance_sensor.lower],
            "allocation_upper": [[None if np.isposinf(x) else float(x) for x in row] for row in body.clearance_sensor.upper],
            "sensor": "all tarsus1..5 collision vertices at q[k], detached mj_kinematics and mj_comPos/material Jacobian; sensor stored at raw rowk+1, held during silence; no authority-cache mutation",
            "native_reflex": {"retraction_rates": [800.,700.], "stumbling_rates": [2200.,1800.], "setpoint_mm": .05, "max_applied_scalar":80., "persistence_threshold":20., "persistence_steps":20},
        ...
        "panel": {
            "development": "unchanged v15+candidate x seeds42/43 x8 =128s",
        ...
        "resource_caps": {"mechanical_seconds": 260, "neural_seconds": 0, "wall_seconds": 14400,
            "rss_bytes": 2 * 1024**3, "evidence_bytes": 8 * 1024**3, "workers": 1, "filesystem_free_floor_bytes":20*1024**3},
    ...
    registration['golden_native_trace']={'path':str(GOLDEN),'sha256':file_sha(GOLDEN),'ticks':[3000,3100],'use':'nonphysical native state/adhesion equivalence only'}
    registration['accepted_v13'] = {'registration_sha256': '407dac972e9754387c3176ef65df1c45935fdc6b1c51e085d973f65924562c5b', 'inventory_sha256':'abb62ad1669545f5435c680aa009f0750b46ffb9f35ecbac1597500ee04a3bbd'}
...
def validate_freeze(root):
    ...
    if file_sha(root/'subjects.json')!=reg['subjects_sha256'] or file_sha(SUBJECTS)!=reg['subjects_sha256']:
        raise ValueError('original subjects changed')
    for group in [reg['graph'],*reg['artifacts'].values()]:
        for path,expected in group.items():
            if file_sha(Path(path))!=expected:raise ValueError('original graph/artifact changed')
    return reg
...
class NativeRecorder:
    def __init__(self, body):
        ...
        self.correction_matrix = correction_matrix(body.controllers[0])
        self.centroids = []
        for geom in self.foot:
    ...
    def row(self, u):
        ...
        native_obs=HybridControllerObservation.from_sim(b.sim,"fly-0")
        core = np.concatenate([
            ...
            getattr(c,"last_clearance",np.zeros(6)),getattr(c,"last_trigger",np.zeros(6)),
            getattr(c,"last_minimum_geom",np.full(6,-1)),getattr(c,"last_minimum_vertex",np.full(6,-1)),
            getattr(c,"last_native_template",d.ctrl[:42]-self.correction_matrix@c.last_info.get("net_corrections",np.zeros(6))),
            np.r_[native_obs.thorax_z,native_obs.tarsus5_z,native_obs.stumbling_contact_forces.ravel(),native_obs.fly_heading],
            getattr(c,"last_allocation",np.zeros((6,7))).ravel(),
            getattr(c,"last_allocation_diagnostics",np.zeros((6,8))).ravel(),
            [getattr(c,"last_observation_tick",-1)],
            getattr(c,"last_relaxation_diagnostics",np.zeros((6,5))).ravel(),
        ])
...
def step(body, u, budget=None):
    observation = HybridControllerObservation.from_sim(body.sim, "fly-0") if float(np.mean(u))>1e-4 else None
    if body.clearance_sensor is not None and float(np.mean(u))>1e-4:
        heights,geoms,vertices=body.clearance_sensor.read(body.data.qpos)
        if isinstance(body.clearance_sensor,AllocationSensor):
            observation=body.clearance_sensor.observation(observation,body.data.qpos,body.tick,heights,geoms,vertices)
        else:raise ValueError("both profiles require allocation observation")
    action = body.controllers[0].step(u, observation)
    apply_locomotion_action(body.sim, "fly-0", action)
    body.drives[0] = u
    if budget is not None:budget.steps+=1
    body.sim.step()
    body.tick += 1
...
def _binding(root, reg):
    return {
        ...
        "generation": "v16-development-seed42-straight02-active-tick10000",
    }
...
def _continuation_sample(body, recorder, u, budget=None):
    step(body, u, budget)
    state_finite(body)
...
def save_active_reference(root, checkpoint):
    """Persist the already captured development reference without integration."""
    samples=checkpoint['expected']
    if len(samples)!=CONTINUATION_TICKS:raise ValueError('incomplete active reference')
    dest=root/'active-reference';dest.mkdir(exist_ok=False)
    offsets=np.r_[0,np.cumsum([len(s['contacts']) for s in samples])]
    np.savez_compressed(dest/'expected.npz',core=np.asarray([s['core'] for s in samples]),
        dense=np.asarray([s['dense'] for s in samples]),integration=np.asarray([s['integration'] for s in samples]),
        contacts=np.concatenate([s['contacts'] for s in samples]),contact_offsets=offsets.astype(np.int64))
    np.savez_compressed(dest/'checkpoint-integration.npz',integration=checkpoint['integration'],drives=checkpoint['drives'])
    (dest/'controller.pkl').write_bytes(checkpoint['controller_bytes'])
    write_json(dest/'checkpoint.json',{'binding':checkpoint['binding'],'integration_shape':list(checkpoint['integration'].shape),
        'integration_dtype':checkpoint['integration'].dtype.str,'controller_sha256':checkpoint['controller_sha256'],
        'controller_schema':checkpoint['controller_schema'],'cache':checkpoint['cache'],
        'native_cache_note':'full authoritative mj_copyData retained in memory for conditional restoration; immutable numeric cache digests retained here'})
    write_json(dest/'expected-digests.json',{'cache':[s['cache'] for s in samples],
        'controller_state_sha256':[s['controller_state_sha256'] for s in samples]})
...
def run(root, anchor):
    ...
    try:
        ...
        save_active_reference(root, restoration["checkpoint"])
        from .analyze_v16 import analyze_panel
        from .verify_v16 import verify_panel
        ...
        if decision['candidate_passed']:
            ...
            from .verify_v16 import verify_repeat
            verify_repeat(root)
            ...
            for _ in range(CONTINUATION_TICKS):
                actual.append(_continuation_sample(body, recorder, np.array([.2, .2]),budget))
            save_restoration(root, checkpoint, actual)
            from .verify_v16 import verify_restoration
