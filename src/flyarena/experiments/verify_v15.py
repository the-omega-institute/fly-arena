"""Independent retained-data v15 verification; never integrates the plant."""
import json
...
def verify_phase_law(core, profile, registration):
    ...
    if True:
        for field in ('phases','magnitudes','targets','ctrl','retraction','stumbling','persistence','net_correction','sensed_clearance','trigger_mask','minimum_geom','minimum_vertex','native_template','allocation','allocation_diagnostics','observation_tick'):
            if not np.array_equal(core[1:,cs[field]][~active],core[:-1,cs[field]][~active]):
                raise ValueError('v15/v12 silence did not hold controller/action exactly')
...
def verify_retraction_and_commands(core,profile,registration):
    ...
    if profile==PROFILE:net[retraction>0]=0
    errors={}
    ...
    for i,leg in enumerate(source.legs):
        ...
        for j,spec in enumerate(source.dofs_per_leg):
            ...
            if profile==PROFILE:corrected[:,index]+=core[:,cs['allocation']].reshape(-1,6,7)[:,i,j]
...
def verify_allocation_row(model,data,pre,post,minima,registration):
    ...
    for leg,(height,geom,vertex) in enumerate(minima):
        ...
        if not (post[cs['retraction']][leg]>0 and 0<phase[leg]<ends[leg]):
            if np.any(offsets[leg]!=0) or np.any(diagnostics[leg]!=0):raise ValueError('strict retraction phase/priority violation')
            continue
        ...
        if authority<=tau:
            ...
            expected=np.zeros(7);diag=np.array([h0,authority,0,budget,-1,0,rank,0])
        else:
            n=w/authority;requested=max(0.,(.05-h0)/authority)
            limits=[(hi[j]-q0[j])/n[j] if n[j]>0 else (lo[j]-q0[j])/n[j] if n[j]<0 else np.inf for j in range(7)]
            limit=min(limits);distance=min(requested,budget,limit)
            expected=distance*n
            ...
            if np.max(np.abs(J[:2]@offsets[leg]))>TOLERANCE or np.linalg.norm(offsets[leg])>budget+TOLERANCE:
                raise ValueError('independent tangent/angular budget violation')
        if np.max(np.abs(expected-offsets[leg]))>TOLERANCE or np.max(np.abs(diag-diagnostics[leg]))>TOLERANCE:
            raise ValueError('independent normal allocation/diagnostics mismatch')
        ...
        for tick in range(ROWS):
            ...
            if profile in (PROFILE,CONTROL) and interval<ROWS and core[interval,cs['input']].mean()>1e-4:
                ...
                if profile==PROFILE:
                    ...
                    verify_allocation_row(model,data,core[tick],core[interval],minima,registration)
                sensor_rows+=1
