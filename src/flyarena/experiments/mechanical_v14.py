"""Prospectively registered v14 whole-foot retraction-trigger experiment."""
...
from .cadence_v14 import ClearanceHybridController, ClearanceObservation, PROFILE, CONTROL
...
APPROVAL = ORIGINAL / "var/sshx/behavior-v14/approved-plan.json"
SUBJECTS = ORIGINAL / "var/sshx/sensorimotor-v7/subjects.json"
BASE = "68988647f2cf4873afe613cc8d1620a7ddf776c5"
...
def new_body(seed, profile):
    ...
    cls = {PROFILE: ClearanceHybridController, CONTROL: ExcursionHybridController}[profile]
    ...
    body.clearance_sensor = WholeFootSensor(body.model) if profile == PROFILE else None
    return body
...
class Budget:
    ...
    def check(self):
        ...
        if (self.steps > 2600000 or report['wall_seconds'] > 14400
            ...
            raise RuntimeError('registered v14 resource cap exceeded')
    def report(self):
        ...
        return {'mechanical_seconds': self.steps*DT, 'full_network_seconds': 0,
                ...
                'evidence_bytes': sum(p.stat().st_size for base in (self.root,Path('/tmp/fly-v14-scratch')) for p in base.rglob('*') if p.is_file()), 'workers': 1}
...
def _controller_schema(controller):
    ...
    for name in ("release_start", "release_end", "last_clearance", "last_trigger", "last_minimum_geom", "last_minimum_vertex"):
...
def freeze(root):
    ...
    sources = {}
    paths = source_files() + [
        ROOT / "pyproject.toml", ROOT / "uv.lock", ROOT / "scripts/report_behavior_v14.py", ROOT / "tests/test_behavior_v14.py",
    ]
    ...
    swing_periods = {leg: [float(c.preprogrammed_steps.swing_period[leg][0]),
                     ...
                     for leg in c.legs}
    registration = {
        "schema": "mechanical-registration/v14", "candidate": PROFILE,
        ...
        "controller": {
            "candidate_equation": "unchanged v12 dtheta=(c/.6)*D and dr=20*(R-r); M=(c>1e-4)&(start<pretheta modulo2pi<end)&(whole-foot prestate clearance<.05mm); independent native retraction state machine per M",
            "only_change_from_v12": "replace single relative-height retraction selector by independent per-leg sensed-clearance swing mask",
            "sensor": "all tarsus1..5 collision vertices at q[k], detached mj_kinematics only; sensor stored at raw rowk+1, held during silence; no authority-cache mutation",
        ...
        "panel": {
            "development": "v12+candidate x seeds42/43 x8 =128s",
...
class NativeRecorder:
    ...
    def row(self, u):
        ...
        core = np.concatenate([
            ...
            d.ctrl[:42]-self.correction_matrix@c.last_info.get("net_corrections",np.zeros(6)),
...
def step(body, u):
    ...
    if body.clearance_sensor is not None and float(np.mean(u))>1e-4:
        ...
        observation=ClearanceObservation(observation,heights,geoms,vertices)
...
def _binding(root, reg):
    return {
        ...
        "generation": "v14-development-seed42-straight02-active-tick10000",
    }
...
def run(root, anchor):
    ...
    try:
        ...
        from .analyze_v14 import analyze_panel
        from .verify_v14 import verify_panel
        ...
        if decision['candidate_passed']:
            ...
            from .verify_v14 import verify_repeat
            verify_repeat(root)
            ...
            from .verify_v14 import verify_restoration
